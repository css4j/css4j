...
changes from the latest tag. For example if you are releasing `5.0.1`:
...
"Maven: deploy css4j 5.0.1 artifacts" or similar but do not push yet.

10) Create a `v<version>` tag in the css4j Git repository. For example:

```shell
cd /path/to/css4j
git tag -s v5.0.1 -m "Release 5.0.1"
...
example we create the 5.0.1 release Zip with the `7z` compressor:
...
cd ..
mv css4j css4j-5.0.1
7z a -mx9 css4j-5.0.1.zip css4j-5.0.1
```
...
./ha.sh /path/to/css4j-5.0.1.zip
```

You should now have the `css4j-5.0.1.sha1`, `css4j-5.0.1.sha256`,
`css4j-5.0.1.b2` and `css4j-5.0.1.md5` files under the `digest` directory.

If the changes to the `css4j.github.io` repo look correct, commit them as
"Digests for css4j 5.0.1 zip distribution" or similar but do not push yet.
...
description like "Latest modular Javadocs after css4j 5.0.1". Make sure that the
