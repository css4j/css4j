...
class GridAreaShorthandSetter extends GridPlacementShorthandSetter {
	...
	public boolean assignSubproperties() {
		...
		while (true) {
			AbstractCSSValue value = gridLine();
			if (value != null) {
				gridLines.add(value);
			} else {
				return false;
		...
		//
		AbstractCSSValue cssval0 = gridLines.item(0);
		AbstractCSSValue cssval;
		switch (gridLines.getLength()) {
		case 1:
			AbstractCSSValue other = omittedValue(cssval0);
	...
	private AbstractCSSValue omittedValue(AbstractCSSValue cssval) {
		AbstractCSSValue other;
