...
abstract class AbstractLexicalValue extends ProxyValue implements CSSLexicalValue {
	...
	public AbstractLexicalValue(Type type) {
		super(type);
	}

	protected AbstractLexicalValue(AbstractLexicalValue copied) {
		super(copied);
		...
		if (lexicalUnit.getParameters() != null) {
			...
			case DIMENSION:
				...
				if (f == 0f) {
					...
					return serializeInfinite(f > 0f, lexicalUnit.getDimensionUnitText());
				}
			...
			case INITIAL:
				...
				break;
			case SYNTAX:
				CSSValueSyntax syntax = lexicalUnit.getSyntax();
				return syntax.toMinifiedString();
			default:
				break;
			}
		}
	...
	private static String serializeInfinite(boolean positive, String unitText) {
		if (positive) {
	...
	public boolean equals(Object obj) {
		...
		AbstractLexicalValue other = (AbstractLexicalValue) obj;
